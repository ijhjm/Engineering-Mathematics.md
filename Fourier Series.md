...
1. 第一項：$\text{奇函數}(x) \times \text{偶函數}(\cos(nx)) = \text{奇函數} \implies$ **積分結果為 0**。
2. 第二項：$\cos(nx)$ 在一整個完整週期 $[-\pi, \pi]$ 上的**積分結果亦為 0**。
...
1. 第二項：$\text{偶函數}(\pi) \times \text{奇函數}(\sin(nx)) = \text{奇函數} \implies$ **積分結果為 0**。
2. 第一項：$\text{奇函數}(x) \times \text{奇函數}(\sin(nx)) = \text{偶函數} \implies$ 可簡化為 **$2$ 倍的 $[0, \pi]$ 區間積分**。
